...
const MemberTableToolbar = ({ table, bulkActionsComponent }) => {
  ...
  return (
    <CompactFilterToolbar end={numSelected > 0 ? bulkActionsComponent : null}>
      ...
      >
        <SelectTrigger className="h-9 w-full sm:w-[140px] text-sm bg-slate-50 border-slate-200 focus:ring-emerald-500">
      ...
      >
        <SelectTrigger className="h-9 w-full sm:w-[130px] text-sm bg-slate-50 border-slate-200 focus:ring-emerald-500">
...
export default function MembersPage() {
  ...
  const { data: statementData = null, isLoading: isStatementLoading } = useSWR(
    selectedMember && isSheetOpen
      ? `/members/${selectedMember.id}/statement`
      : null,
    apiFetcher,
  );
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono bg-white px-2 py-0.5 rounded border border-emerald-100 text-emerald-700">
                        {selectedMember.memberNo ||
                          selectedMember.id.slice(-6).toUpperCase()}
                      </span>
                      <Badge
                        variant={
                      ...
                      <>
                        <div className="grid grid-cols-2 gap-4">
                          <Card className="bg-slate-50 border-none shadow-none p-4 rounded-xl">
                            ...
                            <p
                              className={`text-xl font-bold ${statementData.financial.balance > 0 ? "text-red-600" : "text-emerald-600"}`}
                            >
                              {new Intl.NumberFormat("en-LK", {
                                style: "currency",
                                currency: "LKR",
                              }).format(statementData.financial.balance)}
                            </p>
                            <p className="text-[10px] text-slate-400 mt-1">
                              {statementData.financial.balance > 0
                                ? "Outstanding arrears"
                                : "No pending dues"}
                            </p>
                          </Card>
                          <Card className="bg-slate-50 border-none shadow-none p-4 rounded-xl">
                            ...
                            <p className="text-xl font-bold text-slate-900">
                              {new Intl.NumberFormat("en-LK", {
                                style: "currency",
                                currency: "LKR",
                              }).format(statementData.financial.monthlyRate)}
                            </p>
                            <p className="text-[10px] text-slate-400 mt-1">
                              {selectedMember.paymentFrequency} cycle
                            </p>
                          </Card>
                        </div>
                        ...
                        <div className="space-y-3 pt-2">
                          ...
                          <div className="grid grid-cols-6 sm:grid-cols-12 gap-2">
                            {statementData.financial.monthlyStatus.map(
                              (m, idx) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span
                                      className={`text-[8px] opacity-70 ${textColor}`}
                                    >
                                      {m.year}
                                    </span>
                                  </div>
                        ...
                        <div className="space-y-3 pt-2">
                          ...
                          <div className="space-y-2">
                            {statementData.financial.transactions.length > 0 ? (
                              statementData.financial.transactions
                                .slice()
                                .reverse()
                                .map((tx, idx) => (
                                  <div
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      <div
                                        className={`h-8 w-8 rounded-full flex items-center justify-center ${tx.type === "Payment" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                                      >
                                        {tx.type === "Payment" ? (
                                    ...
                                    <div className="text-right">
                                      <p
                                        className={`text-sm font-bold ${tx.credit > 0 ? "text-emerald-600" : "text-slate-900"}`}
                                      >
                                        {tx.credit > 0
                                          ? `+${tx.credit}`
                                          : `-${tx.debit}`}
