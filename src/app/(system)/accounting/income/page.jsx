...
export default function IncomeSummaryPage() {
  ...
  const handleExport = (type) => {
    ...
    if (type === "csv") {
      ...
      exportToCSV(
        csvData,
        `Income_Summary_${format(new Date(), "yyyy-MM-dd")}.csv`,
      );
    } else if (type === "pdf") {
      const columns = [
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-slate-500">
              Overview of all incoming funds, collections, and donations.
            </p>
          </div>
        ...
        <CompactStatsGrid cols={4}>
          <CompactStatCard
            ...
            sublabel="Combined revenue"
            icon={Wallet}
            iconClassName="text-emerald-600"
            iconBgClassName="bg-emerald-50"
          />
          <CompactStatCard
            label="Sanda Collections"
            value={`Rs. ${stats.sanda.toLocaleString()}`}
            sublabel={`${((stats.sanda / (stats.total || 1)) * 100).toFixed(1)}% of total`}
            icon={CreditCard}
            iconClassName="text-blue-600"
            iconBgClassName="bg-blue-50"
          />
          <CompactStatCard
            label="Donations & Zakat"
            value={`Rs. ${stats.donations.toLocaleString()}`}
            sublabel={`${((stats.donations / (stats.total || 1)) * 100).toFixed(1)}% of total`}
            icon={HandCoins}
          ...
          <CompactStatCard
            label="Other Income"
            value={`Rs. ${(stats.other ?? 0).toLocaleString()}`}
            sublabel={`${(((stats.other ?? 0) / (stats.total || 1)) * 100).toFixed(1)}% of total`}
            icon={Coins}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="rounded-xl border-slate-200 shadow-sm bg-white">
            <CardHeader>
              ...
              <CardDescription>
                Comparison of Sanda vs Donations over time
              </CardDescription>
            </CardHeader>
        ...
        <CompactTableCard
          toolbar={
            ...
            >
              ...
              <div className="relative w-full sm:max-w-[200px] sm:flex-1 sm:min-w-[160px]">
              ...
              >
                <SelectTrigger className="h-9 w-full sm:w-[140px] text-sm bg-slate-50 border-slate-200">
